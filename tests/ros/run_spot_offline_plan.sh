...
# ROS python nodes run from the venv with its one numpy: scripts/setup.sh rebuilt cv_bridge
# against it, and ros_node_pythonpath (scripts/lib/ros_python_env.sh) verifies that at preflight.
# shellcheck disable=SC1091
source "$OPEN_SET_SIM_ROOT/scripts/lib/ros_python_env.sh"
ROS_COMPAT_PYTHONPATH="$(ros_node_pythonpath "$OPEN_SET_PYTHON" 2>/dev/null || printf '%s' "${PYTHONPATH:-}")"
...
check "cv_bridge loads under the venv's numpy (one numpy; scripts/setup.sh rebuilds it)" "ros_node_pythonpath $OPEN_SET_PYTHON"
if ((FAKE)); then
